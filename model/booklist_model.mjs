...
async function addBook(newBook, username) {
    try {
        ...
        const [book, created] = await Book.findOrCreate({where:{title: newBook.title,author: newBook.author}});
...
async function deleteBook(book, username) {
    try {
        ...
        //await bookToRemove.removeBook(user)
        await user.removeBook(bookToRemove) // εναλλακτικά, από την πλευρά του User
...
//κανε ενημέρωση το σχόλιο του χρήστη
const updatecomment = async (req, res, next) => {
    
    const title1 = req.body.newBookTitle;
    const username = req.session.username;
    const comment1 = req.body.BookComment;
    
    try{
        await BookUser.update(
        {comment: comment1},{where:{UserName :username, BookTitle:title1}})
    }
    catch (error) {
...
export { addUser, login, loadBooks, addBook, deleteBook, updatecomment }